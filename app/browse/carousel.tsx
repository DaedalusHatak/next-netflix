...
export default function Carousel({
    ...
    const touchDevice:any =   'ontouchstart' in window || navigator.maxTouchPoints > 0;
...
function changeOrient(){
  ...
  function isTouchDevice(): boolean {
		...
		if (emitImage) {
			...
			if (!touchDevice) {
				hoverTimer = setTimeout(() => {
					slide(slideElement);
					if (e.target) position(target.getBoundingClientRect());
				}, timer);
			}
		}
	}
	...
	function touchStart(event: React.TouchEvent<HTMLDivElement>) {	setTouchMovement({ ...touchMovement, start: event.touches[0].clientX });
	}
