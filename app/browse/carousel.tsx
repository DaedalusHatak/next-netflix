...
export default function Carousel({data,size,overflow,padding,buttonWidth,buttonPadding,buttonVisible,emitImage,emitImageDelay,sm,md,lg,xl,xxl}:any) {
    ...
    function touchEnd() {
      if (
        touchMovement.start > touchMovement.end &&
        touchMovement.end > 0
      )
        forward();
      else if (
  ...
  return (
    ...
    className={`${overflow ? '' : 'overflow'} ${styles.carousel}` }
  >
    <button
     
      style={
       { left: `${buttonPadding}rem`,
      ...
      className={`${styles.handle} ${styles["left-handle"]} ${hoverButtons || buttonVisible ? '' : 'button-visible'}`}
...
{isMobile && wasTriggered &&     <div
    ...
    //   @touchend.passive="touchEnd"
      className={`${styles["slider-wrapper"]} ${carouselMove === 1 || carouselMove === -1 ? 'animate' : ''}`}
    >
 {slides.map(slide=>{
          <div
...
{!isMobile &&     <button
      style={
        {right: `${buttonPadding}rem`,
        width: `${buttonWidth}rem`,}
      }
      onMouseOver={()=>setHoverButtons(true)}
      onMouseLeave={()=>setHoverButtons(false)}
      onClick={()=>forward()}
      className={`${styles["handle"]} ${styles["right-handle"]} ${hoverButtons || buttonVisible ? '' : 'button-visible'}`}
