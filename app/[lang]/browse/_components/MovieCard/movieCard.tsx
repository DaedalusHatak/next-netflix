...
export default function MovieCard({
  ...
  const positionX = () => {
    ...
    if (position.width + 48 > position.x) return position.x;
    return position.x;
  };
  const originX = () => {
    if (position.x > window.innerWidth - position.width - 96 - 48) {
  ...
  return (
    <>
      <Head>
        <link
          rel="preload"
          href={`https://image.tmdb.org/t/p/w500${slide.backdrop_path}`}
          as="image"
        ></link>
      </Head>
      <AnimatePresence>
        {position.width && slide && (
          <motion.div
            ...
            animate={{
              ...
              top: scroll,
              translateY: "-25%",
            }} // Use state to control the scale
          ...
          >
            ...
            {position && slide && (
              <div className={styles.info}>
                ...
                <p className={styles.para}>
                  <span className={styles.name}>Rating: </span>{" "}
                  {slide.vote_average}
                  <span className={`${styles.name} ${styles.gap}`}>
                    Votes:{" "}
                  </span>{" "}
                  {slide.vote_count}
                </p>
